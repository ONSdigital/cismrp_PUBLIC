...
testthat::test_that("Function outputs list", {
  output <- check_dual_parameters_are_not_both_true(dummy_config)

  testthat::expect_type(output, "list")
})

# Test unlisted output has length of 2
testthat::test_that("Function outputs 2 responses", {
  output <- check_dual_parameters_are_not_both_true(dummy_config)
...
testthat::test_that("Function confirms params are not both set to true", {
  output <- check_dual_parameters_are_not_both_true(dummy_config)
  # Function returns TRUE 2 times confirming params are not both set to true
  output <- unlist(output)
  output
  output <- toString(output)
  output
  testthat::expect_setequal(output, "TRUE, TRUE")
})
