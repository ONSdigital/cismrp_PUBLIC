...
testthat::test_that("Function outputs list", {
  output <- check_end_date_occurs_after_prev_end_date(dummy_config)
  testthat::expect_type(output, "list")
})

testthat::test_that("Function outputs 4 responses", {
  output <- check_end_date_occurs_after_prev_end_date(dummy_config)
  output <- unlist(output)
  testthat::expect_length(output, 4)
})

testthat::test_that("Function confirms end date occurs after previous end date with TRUE output for each country", {
  # Function returns TRUE 4 times (one for each country) confirming end date occurs after previous end date
  output <- check_end_date_occurs_after_prev_end_date(dummy_config)
  output <- unlist(output)
  output <- toString(output)

  testthat::expect_setequal(output, "TRUE, TRUE, TRUE, TRUE")
})
