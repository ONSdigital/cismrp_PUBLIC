...
# Function returns TRUE confirming end date occurs before data run date
testthat::test_that("Function confirms end date occurs before data run date with TRUE output", {
  output <- check_end_date_occurs_after_data_run_by_vars(country = "Scotland", "data_run", "end_date", dummy_config)
  testthat::expect_true(output)
})

# Function errors when end date occurs after data run date

testthat::test_that("Function errors when end date occurs after data run date", {
  dummy_config$run_settings$end_date$Scotland <- "20221206"

  testthat::expect_error(check_end_date_occurs_after_data_run_by_vars(country = "Scotland", "data_run", "end_date", dummy_config))
})
