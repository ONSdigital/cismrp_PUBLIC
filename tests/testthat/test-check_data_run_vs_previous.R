...
# Data has data run that occurs after the previous run
testthat::test_that("Data run occurs after previous data run", {
  testthat::expect_gt(as.numeric(dummy_config$run_settings$data_run), as.numeric(dummy_config$run_settings$prev_data_run))
})

# Function returns TRUE confirming data run occurs after previous data run
testthat::test_that("Function confirms data run occurs after previous data run with TRUE output", {
  output <- check_data_run_vs_previous(dummy_config)

  testthat::expect_true(output)
})

testthat::test_that("Function errors when prev data run occurs after data run date", {
  dummy_config$run_settings$prev_data_run <- "20221206"

  testthat::expect_error(check_data_run_vs_previous(dummy_config))
})
