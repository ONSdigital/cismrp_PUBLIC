...
output_post_stratify_draws <- post_stratify_draws(test_pop_tables$Scotland1, posterior_probabilities$Scotland1$All, dummy_country_configs_sample$Scotland$run_settings$n_days_to_model)

testthat::test_that(
  desc = "function returns a matrix",
  code = {
    expect_true(is.matrix(output_post_stratify_draws))
  }
)

testthat::test_that("function returns object of type double", {
  expect_type(output_post_stratify_draws, "double")
})
