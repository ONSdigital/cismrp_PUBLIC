...
testthat::test_that("Function outputs list", {
  output <- check_end_date_occurs_after_data_run(dummy_config)
  testthat::expect_type(output, "list")
})

# Test unlisted output has length of 8

testthat::test_that("Function outputs 8 responses", {
  output <- check_end_date_occurs_after_data_run(dummy_config)
  output <- unlist(output)
  testthat::expect_length(output, 8)
})


testthat::test_that("Function confirms end date occurs after data run with TRUE output for each country end dates vs dataruns", {
  # Function returns TRUE 8 times confirming end dates (previous and current) occurs before data runs (both previous and current )
  output <- check_end_date_occurs_after_data_run(dummy_config)
  output <- unlist(output)
  output <- toString(output)

  testthat::expect_setequal(output, "TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE")
})
